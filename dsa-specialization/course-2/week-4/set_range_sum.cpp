...
void traverse_pre_order(Vertex * n){
  if(n == NULL) return;
  cout<< "node: " << n->key << " ";
...
void traverse(Vertex * n){
  traverse_pre_order(n);
  cout << std::endl;
}
...
long long sum(int from, int to) {
  ...
  if(middle != NULL)
    ans = middle->sum;
  //traverse_pre_order(middle);
  
  Vertex * leftMiddle = merge(left, middle);
  root = merge(leftMiddle, right);
  return ans;  
}
